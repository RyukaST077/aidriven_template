...
1. `docs/01_Project_Design/05_Feature_List.md`（機能一覧）を読み込み、機能ID/画面ID/帳票ID/IF ID/バッチIDを把握する。
2. `docs/01_Project_Design/03_Architecture.md`（アーキテクチャ設計）を読み込み、技術スタック・設計方針を把握する。
3. `docs/01_Project_Design/04_Basic_Design.md`（基本設計）を読み込み、システム構成・モジュール構成を把握する。
4. 最初の回答は以下を必ず含める：
