...
1. `docs/01_Project_Design/01_Requirements.md`（要件定義）を読み込み、セキュリティ関連のNFR/CONを把握する。
2. `docs/01_Project_Design/02_Business_Process.md`（業務プロセス）を読み込み、ロール・権限・機密データの流れを把握する。
3. `docs/01_Project_Design/03_Architecture.md`（アーキテクチャ設計）を読み込み、技術スタック・システム構成を把握する。
4. `docs/01_Project_Design/04_Basic_Design.md`（基本設計）を読み込み、認証・認可の基本方針を把握する。
5. 最初の回答は以下を必ず含める：
