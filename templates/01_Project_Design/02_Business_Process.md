...
| プロセスID | プロセス名 | 開始条件 | 終了条件 | 主要ロール | 関連機能（F-ID） | 優先度 | 備考 |
| ------ | ----- | ---- | ---- | ----- | ---------- | --- | -- |
| BP-001 |       |      |      |       |            |     |    |
...
| 対象（業務オブジェクト） | 操作 | R-001 | R-002 | R-003 | 判定条件（データ範囲等） | ログ/監査 |
| ------------ | -- | ----: | ----: | ----: | ------------ | ----- |
| 例：申請         | 起票 |     ○ |     — |     — | 自部門          | 操作ログ  |
| 例：申請         | 承認 |     — |     ○ |     — | 金額<=X        | 監査ログ  |
...
| No | アクティビティ（誰が） | 入力 | 処理内容 | 出力 | 関連ルール（BR-ID） | 関連画面/IF | ログ/監査 |
| -: | ----------- | -- | ---- | -- | ------------ | ------- | ----- |
|  1 |             |    |      |    |              |         |       |

##### 4.2.2 例外・分岐フロー

| EX-ID  | 発生条件（トリガー） | 分岐点（No/ACT-ID） | 対応（手戻り/代替/中断） | 通知/エスカレーション | 関連ルール（BR-ID） | 運用回避策 |
| ------ | ---------- | -------------- | ------------- | ----------- | ------------ | ----- |
| EX-001 |            |                |               |             |              |       |
...
* BR-A：業務判断（条件分岐、可否判定）
* BR-C：計算（料金、金額、在庫引当、ポイント等）
* BR-S：状態遷移（ステータス、承認段階）
* BR-T：締め・カットオフ（営業日、月次、期日）
* BR-R：権限・スコープ（参照範囲、操作可否、代行）
* BR-E：例外・エラー（業務例外、復旧手順）

### 5.2 ルール一覧

| BR-ID  | 名称 | 分類   | 適用範囲（BP/画面/IF） | 概要 |  優先度 | 関連要件ID  | 備考 |
| ------ | -- | ---- | -------------- | -- | ---: | ------- | -- |
| BR-001 |    | BR-A | BP-001         |    | High | REQ-xxx |    |

### 5.3 ルール詳細テンプレ（共通）

#### BR-xxx：＜ルール名＞

* 分類：BR-A/BR-C/BR-S/BR-T/BR-R/BR-E
* 目的：
...
| 現状態 | イベント/操作 | 遷移先 | 許可ロール | 条件（BR-ID） | 副作用（通知/IF/更新） |
| --- | ------- | --- | ----- | --------- | ------------- |
| 下書き | 提出      | 申請中 | R-001 | BR-0xx    | 通知A、履歴        |
...
| BP/BR/EX | ID     | 要件ID（REQ） | 機能ID（F） | 画面ID    | 帳票ID    | IF-ID  | データ項目ID | テストID  |
| -------- | ------ | --------- | ------- | ------- | ------- | ------ | ------- | ------ |
| BP       | BP-001 | REQ-010   | F-023   | SCR-005 | RPT-002 | IF-001 | D-123   | TC-001 |
